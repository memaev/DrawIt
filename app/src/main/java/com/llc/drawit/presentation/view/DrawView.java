...
public class DrawView extends View {
    ...
    private DrawingInstrument currentInstrument = DrawingInstrument.PEN; //по умолчанию инструмент - ручка (рисование)
    ...
    public DrawView(Context context, AttributeSet attrs) {
        ...
        textPaint.setTextSize(70); //размер текста, который можно вставить на доску
    ...
    protected void onDraw(@NonNull Canvas canvas) {
        ...
        paths.forEach((stroke, points) -> {
            //определяем что мы должны отобразить: текст или линию(рисунок)
            if (stroke.text != null) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                //определяем, что пользователь хочет добавить: текст или нарисовать рисунок
